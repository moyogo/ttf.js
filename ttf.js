...
(function() {
  var AnchorTable, AttachmentListTable, BaseArray, CaretValueTable, ChainContextPositioning, ChainPosClassRule, ChainPosClassSet, ChainPosRule, ClassDefinitionTable, CmapSubTable, CmapTable, Common, CompositeGlyphData, CoverageTable, CursiveAttachment, DeviceTable, ExtensionPositioning, FeatureListTable, FeatureTable, GDEFTable, GPOSLookupType, GPOSTable, GSUBLookupType, GlyfTable, Glyph, HeadTable, HheaTable, HmtxTable, LangSys, LigCaretListTable, LigatureArray, LigatureAttach, LocaTable, Lookup, LookupListTable, LookupTable, Mark2Array, MarkArray, MarkGlyphSetsDef, MarkToBaseAttachment, MarkToLigatureAttachment, MarkToMarkAttachment, MaxpTable, NameTable, OS_2Table, PairAdjustment, PairSet, PostTable, ScriptListTable, ScriptTable, SimpleGlyphData, SingleAdjustment, TTFDataView, TTFString, TrueType, ValueRecord, jDataView, standardNames, ttfjs, valueFormatLength;
  ...
  ScriptListTable = (function() {
    ...
    ScriptListTable.createFromJSON = function(json) {
      var scriptListTable, scriptRecord, _i, _len, _ref;
      if (typeof json === 'string') {
        json = JSON.parse(json);
      }
      scriptListTable = new ScriptListTable();
      scriptListTable.scriptCount = json.scriptCount;
      _ref = json.scriptRecord;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        scriptRecord = _ref[_i];
  ...
  ChainPosClassSet = (function() {
    ...
    ChainPosClassSet.createFromTTFDataView = function(view, offset) {
      var chainPosClassRule, chainPosClassRuleCount, chainPosClassRuleOffset, chainPosClassRules, chainPosClassSet, i, _i;
      view.seek(offset);
      chainPosClassSet = new ChainPosClassSet();
      chainPosClassRuleCount = view.getUshort();
      if (chainPosClassRuleCount > 0) {
        chainPosClassRules = [];
        for (i = _i = 0; 0 <= chainPosClassRuleCount ? _i <= chainPosClassRuleCount : _i >= chainPosClassRuleCount; i = 0 <= chainPosClassRuleCount ? ++_i : --_i) {
  ...
  MarkArray = (function() {
    ...
    MarkArray.createFromTTFDataView = function(view, offset) {
      var i, markAnchor, markAnchorOffset, markArray, markClass, markCount, markRecord, markRecords, _i, _ref;
      view.seek(offset);
  ...
  ClassDefinitionTable = (function() {
    ...
    ClassDefinitionTable.createFromTTFDataView = function(view, offset) {
      var classDefTable, classFormat, classId, classRangeCount, classRangeRecords, classValueArray, end, glyphCount, i, j, start, startGlyph, _i, _j, _k, _ref, _ref1;
      view.seek(offset);
      ...
      if (classFormat === 2) {
        ...
        if (classRangeCount > 0) {
          for (i = _j = 0, _ref1 = classRangeCount - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
            ...
            classId = view.getUshort();
            for (j = _k = start; start <= end ? _k <= end : _k >= end; j = start <= end ? ++_k : --_k) {
              classValueArray.push({
                gId: j,
                "class": classId
              });
